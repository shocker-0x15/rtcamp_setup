...
- 制限時間は180秒です。
- 明示的な静止画の提出を廃止しました(詳細は後述)。
- 版権物のレンダリングを禁止します。
- スライドに提出作品よりも明確に高品質な画像を掲載することを禁止します。
...
- Start-Processでプログラムを起動しないでください。\
  (一連の提出作品を実行するスクリプトでプログラム実行前後の時間計測等を行っているため)
